...
a2lix_lib.sfCollection = (() => {
	...
	const processCollectionElt = (collectionElt, manageRemoveEntry = false, lang) => {
		...
		collectionElt.addEventListener("click", (evt) => configureCollectionElt(evt, manageRemoveEntry, lang));
	};

	const appendEntryAddLink = (collectionElt, lang) => {
		// Allow custom label
		const entryLabel = collectionElt.getAttribute("data-entry-add-label") || "",
			entryLabelClass = collectionElt.getAttribute("data-entry-add-class") || "btn btn-primary btn-sm mt-2";

		const entryAddLink = getButtonElt(`${lang.add} ${entryLabel}`, "add", `${entryLabelClass}`);
		collectionElt.appendChild(entryAddLink);
	};

	const appendEntryRemoveLink = (collectionElt, lang) => {
		const entryLabel = collectionElt.getAttribute("data-entry-remove-label") || "",
			entryLabelClass = collectionElt.getAttribute("data-entry-remove-class") || "btn btn-danger btn-sm";

		const entryRemoveLink = getButtonElt(`${lang.remove} ${entryLabel}`, "remove", `${entryLabelClass}`);
	...
	const addEntry = (collectionElt, entryAddButton, manageRemoveEntry, lang) => {
		...
		if (manageRemoveEntry) {
			const entryLabel = collectionElt.getAttribute("data-entry-remove-label") || "",
				entryLabelClass = collectionElt.getAttribute("data-entry-remove-class") || "btn btn-danger btn-sm",
				entryRemoveLink = getButtonElt(`${lang.remove} ${entryLabel}`, "remove", `${entryLabelClass}`);
	...
	const getTemplateContent = (entryPrototype, entryIndex) => {
		...
		const entryHtml = entryPrototype.replace(/__name__label__/g, `!New! ${entryIndex}`).replace(/__name__/g, entryIndex);
